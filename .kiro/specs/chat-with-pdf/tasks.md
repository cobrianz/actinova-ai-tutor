...
- [ ] 2. Create `src/app/lib/pdfExtractor.js`
  ...
  - [-] 2.2 Implement text-layer extraction pass (steps 1–5 of the algorithm in design §3)
  ...
  - [~] 2.3 Implement OCR pass via Tesseract.js (steps 6–10 of the algorithm in design §3)
...
- [ ] 3. Create `src/app/api/chat/pdf/route.js`
  - [~] 3.1 Implement request validation and `buildSystemPrompt` helper
  ...
  - [~] 3.2 Implement `handlePost` and export `POST` with the middleware stack
...
- [~] 4. Checkpoint — verify extraction and API independently
...
- [ ] 5. Create `src/app/components/ChatWithPDF.jsx`
  - [~] 5.1 Scaffold component state, session setup, and `beforeunload` cleanup
  ...
  - [~] 5.3 Implement extraction phase — call `pdfExtractor.js` and show progress
  ...
  - [~] 5.4 Implement chat phase UI — file info bar, message list, and input bar
  ...
  - [~] 5.5 Implement `handleSend` — credit gate, API call, and error handling
...
- [ ] 7. Modify `src/app/components/DashboardContent.jsx`
  - [~] 7.1 Add `ChatWithPDF` import and `chat-pdf` route mapping
  ...
  - [~] 7.2 Add `isPdfChat` flag and extend full-height layout conditions
...
- [ ] 8. Modify `src/app/components/Sidebar.jsx`
  - [~] 8.1 Add "Chat with PDF" nav item to the "Create" group
