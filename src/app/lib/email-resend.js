// lib/email-resend.js
// Email service using Resend (easier setup than SMTP)
...
export async function sendVerificationEmail(
  ...
  try {
    // If no Resend API key, fall back to console logging
    if (!process.env.RESEND_API_KEY) {
      console.log("\n📧 EMAIL VERIFICATION (No API Key - Development Mode)");
      console.log("=====================================");
    ...
    const { data, error } = await resend.emails.send({
      from: "Actinova AI Tutor <onboarding@resend.dev>",
      to: [email],
      subject: "🎓 Welcome to Actinova AI Tutor - Verify Your Email",
      html: getEmailVerificationTemplate(firstName, verificationLink),
      text: `Hi ${firstName},\n\nWelcome to Actinova AI Tutor! Please verify your email address by clicking this link: ${verificationLink}\n\nIf you didn't create an account, please ignore this email.\n\nBest regards,\nThe Actinova AI Tutor Team`,
    });
...
export async function sendPasswordResetEmail(email, firstName, resetToken) {
  try {
    // If no Resend API key, fall back to console logging
    if (!process.env.RESEND_API_KEY) {
      console.log("\n📧 PASSWORD RESET EMAIL (No API Key - Development Mode)");
      console.log("=====================================");
      console.log(`To: ${email}`);
...
export async function testEmailConnection() {
  try {
    if (!process.env.RESEND_API_KEY) {
      console.log("✅ Email service is ready (development mode - no API key)");
      return { success: true };
    }
