...
import {
  ...
  ChevronDown, ChevronUp, Target, FileText, Lock, Unlock, ExternalLink, Award
} from "lucide-react";
import { TYPE_CONFIG } from "../constants";

export default function CourseTab({ classroomState }) {
  const { classroom, announcements, forkedContent, isForkedContentLocked, handleToggleForkUnlock } = classroomState;
      ...
      {classroom.modules?.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Course Modules ({classroom.modules.length})</h3>
          </div>
          <div className="space-y-2">
            {classroom.modules.map((mod, i) => (
