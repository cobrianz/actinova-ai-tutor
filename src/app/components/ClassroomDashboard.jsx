...
import {
  ...
  Bell, ClipboardList, BookMarked, Layers, Info, Globe,
} from "lucide-react";
...
function CreateClassroomForm({ onClose, onCreated }) {
  const [form, setForm] = useState(() => {
    ...
    return { name: "", description: "", subject: "", maxStudents: 50, semester: "", academicLevel: "undergraduate", gradingScheme: "percentage", prerequisites: "", syllabus: "" };
  });
  const [loading, setLoading] = useState(false);
  const [aiLoading, setAiLoading] = useState(false);
  const [aiSyllabusLoading, setAiSyllabusLoading] = useState(false);

  useEffect(() => { localStorage.setItem("create_classroom_draft", JSON.stringify(form)); }, [form]);
  ...
  const handleGenerateSyllabus = async () => {
    ...
    try {
      const res = await apiClient.post("/api/classrooms/ai-generate", { task: "syllabus", name: form.name, subject: form.subject, content: form.description });
      const data = await res.json();
  ...
  const handleCreate = async () => {
    ...
    try {
      const payload = {
        name: form.name, description: form.description, subject: form.subject, maxStudents: parseInt(form.maxStudents) || 50,
        semester: form.semester, academicLevel: form.academicLevel, gradingScheme: form.gradingScheme,
      ...
      const data = await res.json();
      if (data.success) { toast.success("Classroom created!"); localStorage.removeItem("create_classroom_draft"); onCreated(data.classroom); }
  ...
  return (
    <div className="space-y-4">
      <button onClick={onClose} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-[11px] font-medium transition-colors group">
      ...
      <div className="min-h-[50vh] flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500">Create a classroom and invite students</p>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 space-y-4">
            <div>
              ...
              <input value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="e.g. CS 101 — Intro to Programming" className={inputCls} />
            </div>
            ...
            <div>
              ...
              <textarea value={form.syllabus} onChange={(e) => update("syllabus", e.target.value)} placeholder="Course syllabus..." rows={5} className={inputCls + " resize-none"} />
            </div>
            <button onClick={handleCreate} disabled={loading || !form.name.trim()} className="w-full py-2.5 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 disabled:opacity-50 transition-colors">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Chapter 1 Review" className={inputCls} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min={1} max={100} value={form.maxAttempts} onChange={(e) => setForm({ ...form, maxAttempts: parseInt(e.target.value) || 1 })} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Description</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Brief description..." rows={2} className={inputCls + " resize-none"} />
          </div>
          <div>
            <div className="flex items-center justify-between mb-1.5">
          ...
          <div>
            ...
            {loadingCourses ? (
              <div className="h-10 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg animate-pulse" />
            ) : courses.length > 0 ? (
              <div className="space-y-1.5">
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                  <input value={courseSearch} onChange={(e) => setCourseSearch(e.target.value)} placeholder="Search your courses..." className="w-full pl-8 pr-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500/30" />
                </div>
                <div className="max-h-32 overflow-y-auto space-y-1 scrollbar-hide">
                  {filteredCourses.map((course) => (
                    <button key={course._id || course.id} onClick={() => setForm({ ...form, courseId: form.courseId === (course._id || course.id) ? "" : (course._id || course.id) })}
                      ...
                      <span className="text-xs text-slate-700 dark:text-slate-300 truncate flex-1">{course.title}</span>
...
function ClassroomCard({ classroom, role, onClick, onDelete }) {
  ...
  const stats = role === "instructor"
    ? [{ label: "Students", value: classroom.studentCount, icon: Users }, { label: "Assignments", value: classroom.assignmentCount, icon: BookOpen }]
    : [{ label: "Assignments", value: classroom.assignmentCount, icon: BookOpen }, { label: "Due Soon", value: classroom.dueAssignments || 0, icon: Clock, color: isOverdue ? "text-amber-500" : "" }];
    ...
    <motion.div whileHover={{ y: -2, boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }} onClick={onClick}
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="px-1.5 py-0.5 bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 text-[9px] font-bold rounded-full uppercase tracking-wider flex-shrink-0">{classroom.semester || "Active"}</span>
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        {stats.map(({ label, value, icon: Icon, color }, i) => (
          <span key={i} className="flex items-center gap-1 text-[10px] text-slate-500">
            <span className={`w-1.5 h-1.5 rounded-full ${color ? "bg-amber-400" : "bg-green-400"}`} />
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
        ...
        <span className="text-green-600 dark:text-green-400 text-xs font-semibold flex items-center gap-0.5 group-hover:gap-1.5 transition-all">
...
function ClassroomDetail({ classroom, onBack, user }) {
  ...
  const [matAiLoading, setMatAiLoading] = useState(null);
  ...
  const fetchDiscussions = useCallback(async () => {
    ...
    try { const res = await apiClient.get(`/api/classrooms/${classroom.id}/discussions`); const data = await res.json(); if (data.success) setDiscussions(data.discussions); } catch {} finally { setDiscussionsLoading(false); }
  }, [classroom.id]);

  const fetchPosts = useCallback(async (discussionId) => {
    setPostsLoading(true);
    try { const res = await apiClient.get(`/api/classrooms/${classroom.id}/discussions/${discussionId}/posts`); const data = await res.json(); if (data.success) setDiscussionPosts(data.posts); } catch {} finally { setPostsLoading(false); }
  }, [classroom.id]);

  const fetchNotes = useCallback(async () => {
    setNotesLoading(true);
    try { const res = await apiClient.get(`/api/classrooms/${classroom.id}/notes`); const data = await res.json(); if (data.success) setNotes(data.notes); } catch {} finally { setNotesLoading(false); }
  }, [classroom.id]);

  const fetchMaterials = useCallback(async () => {
    setMaterialsLoading(true);
    try { const res = await apiClient.get(`/api/classrooms/${classroom.id}/materials`); const data = await res.json(); if (data.success) setMaterials(data.materials); } catch {} finally { setMaterialsLoading(false); }
  }, [classroom.id]);

  const fetchStudents = useCallback(async () => {
    if (!isInstructor) return;
    setLoading(true);
    try { const res = await apiClient.get(`/api/classrooms/${classroom.id}/students`); const data = await res.json(); if (data.success) { setStudents(data.students); setStudentStats(data.stats); } } catch {} finally { setLoading(false); }
  ...
  const handleMarkComplete = async (assignmentId) => {
    try {
      ...
      } else { toast.error(data.error || "Failed to update progress"); }
  ...
  const handleStartAssignment = async (assignmentId) => {
    try {
      ...
      const data = await res.json();
      if (data.success) { setAssignments((prev) => prev.map((a) => a.id === assignmentId ? { ...a, myProgress: { ...a.myProgress, status: "in_progress", progress: 10 } } : a)); toast.success("Assignment started!"); }
  ...
  const handleCreateDiscussion = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setDiscussions([data.discussion, ...discussions]); setNewDiscTitle(""); setNewDiscDesc(""); setShowNewDiscussion(false); toast.success("Discussion created!"); }
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setDiscussionPosts([...discussionPosts, data.post]); setReplyContent(""); setReplyingTo(null); setDiscussions((prev) => prev.map((d) => (d._id || d.id) === (selectedDiscussion._id || selectedDiscussion.id) ? { ...d, postCount: (d.postCount || 0) + 1, lastActivityAt: new Date().toISOString() } : d)); }
      else { toast.error(data.error || "Failed"); }
  ...
  const handleGenerateDiscussionPrompt = async () => {
    ...
    try {
      const res = await apiClient.post("/api/classrooms/ai-generate", { task: "discussion_prompt", name: newDiscTitle || "general topic", subject: classroom.subject, classroomName: classroom.name });
      const data = await res.json();
      if (data.result) { setNewDiscTitle(newDiscTitle || "Discussion"); setNewDiscDesc(data.result); toast.success("Prompt generated!"); }
  ...
  const handleCreateNote = async () => {
    ...
    try {
      const res = await apiClient.post(`/api/classrooms/${classroom.id}/notes`, { title: newNoteTitle, content: newNoteContent, tags: newNoteTags.split(",").map((t) => t.trim()).filter(Boolean) });
      const data = await res.json();
      if (data.success) { setNotes([data.note, ...notes]); setNewNoteTitle(""); setNewNoteContent(""); setNewNoteTags(""); setShowNewNote(false); toast.success("Note created!"); }
  ...
  const handleGenerateNote = async () => {
    ...
    try {
      const res = await apiClient.post("/api/classrooms/ai-generate", { task: "generate_note", name: newNoteTitle, subject: classroom.subject, classroomName: classroom.name });
      const data = await res.json();
      if (data.result) { setNewNoteContent(data.result); toast.success("Note generated!"); }
  ...
  const handleSummarizeNote = async (note) => {
    ...
    try {
      const res = await apiClient.post("/api/classrooms/ai-generate", { task: "note_summary", content: note.content, classroomName: classroom.name });
      const data = await res.json();
      if (data.result) { setNotes((prev) => prev.map((n) => (n._id || n.id) === (note._id || note.id) ? { ...n, content: data.result } : n)); toast.success("Note summarized!"); }
    } catch { toast.error("Failed to summarize"); } finally { setNoteSummaryLoading(null); }
  };
  ...
  const handleCreateMaterial = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setMaterials([data.material, ...materials]); setNewMat({ title: "", description: "", type: "document", url: "", weekNumber: 0, category: "", isRequired: false }); setShowNewMaterial(false); toast.success("Material added!"); }
  ...
  const handlePostAnnouncement = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setAnnouncements([...announcements, data.announcement]); setNewAnnTitle(""); setNewAnnContent(""); setShowNewAnnouncement(false); toast.success("Announcement posted!"); }
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const payload = { name: settingsForm.name, subject: settingsForm.subject, description: settingsForm.description, semester: settingsForm.semester, academicLevel: settingsForm.academicLevel, gradingScheme: settingsForm.gradingScheme, syllabus: settingsForm.syllabus, schedule: settingsForm.schedule, prerequisites: settingsForm.prerequisites, settings: settingsForm.settings };
  ...
  const tabs = [
    ...
    ...(isInstructor ? [{ id: "students", label: "Students", icon: Users }] : []),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        <button onClick={onBack} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          <ChevronRight className="w-5 h-5 rotate-180" />
        </button>
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white truncate">{classroom.name}</h2>
          <p className="text-xs text-slate-500">{classroom.subject || "No subject"} {classroom.semester ? `· ${classroom.semester}` : ""}</p>
        </div>
        {isInstructor && (
          <div className="flex items-center gap-2">
            <button onClick={() => setShowInvite(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors">
      ...
      <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg p-1 overflow-x-auto scrollbar-hide">
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setActiveTab(id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all flex-shrink-0 justify-center min-w-0 ${activeTab === id ? "bg-white dark:bg-slate-700 text-green-600 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}>
            <Icon className="w-3.5 h-3.5 flex-shrink-0" /><span className="hidden sm:inline">{label}</span>
          </button>
        ))}
      </div>
      ...
      {activeTab === "assignments" && (
        <div className="space-y-3">
          {isInstructor && (
            <div className="flex items-center gap-2">
              <button onClick={() => setShowCreateAssignment(true)} className="flex items-center gap-2 flex-1 p-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-500 hover:border-green-400 hover:text-green-600 transition-colors">
                <Plus className="w-4 h-4" /> Add Assignment
              </button>
              {isInstructor && (
                <button onClick={() => setShowNewAnnouncement(!showNewAnnouncement)} className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-amber-200 dark:border-amber-500/30 rounded-xl text-sm text-amber-600 hover:border-amber-400 transition-colors">
          ...
          {isInstructor && showNewAnnouncement && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={sectionCls}>
              ...
              <div className="flex gap-2">
                <button onClick={handlePostAnnouncement} className="px-4 py-2 bg-amber-500 text-white rounded-lg text-xs font-semibold hover:bg-amber-600 transition-colors">Post Announcement</button>
                <button onClick={() => setShowNewAnnouncement(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
              </div>
            </motion.div>
          )}
          {assignments.length === 0 ? (
            ...
            assignments.map((assignment) => {
              ...
              return (
                <motion.div key={assignment.id} whileHover={{ y: -1 }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-slate-500">
                        ...
                        {assignment.maxAttempts > 1 && <span className="flex items-center gap-0.5"><RefreshIcon />{assignment.maxAttempts} attempts</span>}
                        {progress && (
                          <span className="flex items-center gap-0.5">
                            <span className={`w-1.5 h-1.5 rounded-full ${progress.status === "completed" ? "bg-green-400" : progress.status === "in_progress" ? "bg-amber-400" : "bg-slate-300"}`} />
                            {progress.status === "completed" ? "Done" : progress.status === "in_progress" ? `${progress.progress}%` : "Not started"}
                          </span>
                        )}
                      </div>
                    </div>
                    {!isInstructor ? (
                      <div className="flex-shrink-0 mt-1">
                        {progress?.status === "completed" ? (
                          <span className="flex items-center gap-1 text-[10px] font-semibold text-green-600 bg-green-50 dark:bg-green-500/10 px-2 py-1 rounded-lg"><CheckCircle2 className="w-3 h-3" /> Done</span>
                        ) : progress?.status === "in_progress" ? (
                          <button onClick={(e) => { e.stopPropagation(); handleMarkComplete(assignment.id); }} className="flex items-center gap-1 text-[10px] font-semibold text-white bg-green-500 hover:bg-green-600 px-2 py-1 rounded-lg transition-colors"><Check className="w-3 h-3" /> Mark Complete</button>
                        ) : (
      ...
      {activeTab === "discussions" && (
        <div className="space-y-3">
          {selectedDiscussion ? (
            <div className="space-y-3">
              <button onClick={() => { setSelectedDiscussion(null); setDiscussionPosts([]); }} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-[11px] font-medium transition-colors">
              ...
              <div className={sectionCls}>
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      {selectedDiscussion.isPinned && <Pin className="w-3.5 h-3.5 text-amber-500" />}
                      <h3 className="text-base font-bold text-slate-900 dark:text-white">{selectedDiscussion.title}</h3>
                    </div>
                    {selectedDiscussion.description && <p className="text-xs text-slate-500 mt-1">{selectedDiscussion.description}</p>}
                  </div>
                  {isInstructor && (
                    <div className="flex items-center gap-1">
                      <button onClick={async () => { try { await apiClient.patch(`/api/classrooms/${classroom.id}/discussions/${selectedDiscussion._id || selectedDiscussion.id}`, { isPinned: !selectedDiscussion.isPinned }); setSelectedDiscussion({ ...selectedDiscussion, isPinned: !selectedDiscussion.isPinned }); fetchDiscussions(); } catch {} }}
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-amber-500 transition-colors"><Pin className="w-3.5 h-3.5" /></button>
                      <button onClick={async () => { try { await apiClient.patch(`/api/classrooms/${classroom.id}/discussions/${selectedDiscussion._id || selectedDiscussion.id}`, { isClosed: !selectedDiscussion.isClosed }); setSelectedDiscussion({ ...selectedDiscussion, isClosed: !selectedDiscussion.isClosed }); } catch {} }}
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-blue-500 transition-colors">{selectedDiscussion.isClosed ? <Unlock className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5" />}</button>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  {discussionPosts.map((post) => (
                    <div key={post._id || post.id} className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 ${post.parentPostId ? "ml-8 border-l-2 border-l-green-400" : ""}`}>
                      ...
                      {!selectedDiscussion.isClosed && (
                        <button onClick={() => setReplyingTo(post._id || post.id)} className="mt-2 text-[10px] font-semibold text-green-600 hover:text-green-700 transition-colors">Reply</button>
                      )}
                      {replyingTo === (post._id || post.id) && (
                        <div className="mt-3 flex gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                            <button onClick={handleCreatePost} className="p-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"><Send className="w-3.5 h-3.5" /></button>
                            <button onClick={() => { setReplyingTo(null); setReplyContent(""); }} className="p-2 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"><X className="w-3.5 h-3.5" /></button>
                          </div>
                        </div>
                      )}
                    </div>
                  ))}
                  {discussionPosts.length === 0 && <p className="text-sm text-slate-400 text-center py-8">No posts yet. Start the conversation!</p>}
                </div>
              )}
              {!selectedDiscussion.isClosed && (
                <div className="flex gap-2">
                  <textarea value={replyContent} onChange={(e) => setReplyContent(e.target.value)} placeholder="Write a post..." rows={2} className={inputCls + " resize-none flex-1"} onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) handleCreatePost(); }} />
                  <button onClick={handleCreatePost} disabled={!replyContent.trim()} className="self-end px-4 py-2.5 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 disabled:opacity-50 transition-colors"><Send className="w-4 h-4" /></button>
                </div>
              )}
            ...
            <>
              {isInstructor && (
                <div className="flex gap-2">
                  <button onClick={() => setShowNewDiscussion(!showNewDiscussion)} className="flex items-center gap-2 flex-1 p-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-500 hover:border-green-400 hover:text-green-600 transition-colors">
                    <Plus className="w-4 h-4" /> New Discussion
                  </button>
                  <button onClick={handleGenerateDiscussionPrompt} disabled={discAiLoading} className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-green-200 dark:border-green-500/30 rounded-xl text-sm text-green-600 hover:border-green-400 transition-colors disabled:opacity-50">
                    {discAiLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} AI Prompt
                  </button>
                </div>
              )}
              {showNewDiscussion && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={sectionCls}>
                  ...
                  <div><div className="flex items-center justify-between mb-1.5"><label className={labelCls}>Description</label>
                    <button onClick={handleGenerateDiscussionPrompt} disabled={discAiLoading} className="flex items-center gap-1 text-[10px] font-semibold text-green-600 hover:text-green-700 disabled:opacity-40 transition-colors">
                      {discAiLoading ? <Loader2 size={11} className="animate-spin" /> : <Sparkles size={11} />} Generate Prompt
                    </button>
                  </div>
                    <textarea value={newDiscDesc} onChange={(e) => setNewDiscDesc(e.target.value)} placeholder="Description or prompt..." rows={3} className={inputCls + " resize-none"} />
                  </div>
                  <div className="flex gap-2">
                    <button onClick={handleCreateDiscussion} className="px-4 py-2 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors">Create Discussion</button>
                    <button onClick={() => setShowNewDiscussion(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
                  </div>
                </motion.div>
              )}
              {discussionsLoading ? (
                ...
                <EmptyState icon={MessageSquare} title="No discussions yet" description={isInstructor ? "Start a discussion for your students" : "No discussions have been posted yet"} action={isInstructor ? "New Discussion" : undefined} onAction={() => setShowNewDiscussion(true)} />
              ) : (
                discussions.map((disc) => (
                  <motion.div key={disc._id || disc.id} whileHover={{ y: -1 }} onClick={() => setSelectedDiscussion(disc)}
                    className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 hover:border-green-300 dark:hover:border-green-600 transition-colors cursor-pointer">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          {disc.isPinned && <Pin className="w-3 h-3 text-amber-500 flex-shrink-0" />}
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-slate-500">
                          <span className="flex items-center gap-0.5"><MessageSquare className="w-3 h-3" /> {disc.postCount || 0} posts</span>
                          {disc.lastActivityAt && <span className="flex items-center gap-0.5"><Clock className="w-3 h-3" /> {new Date(disc.lastActivityAt).toLocaleDateString()}</span>}
                        </div>
                      </div>
      ...
      {activeTab === "notes" && (
        <div className="space-y-3">
          {isInstructor && (
            <button onClick={() => setShowNewNote(!showNewNote)} className="flex items-center gap-2 w-full p-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-500 hover:border-green-400 hover:text-green-600 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={sectionCls}>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button onClick={handleGenerateNote} disabled={noteAiLoading || !newNoteTitle.trim()} className="flex items-center gap-1 text-[10px] font-semibold text-green-600 hover:text-green-700 disabled:opacity-40 transition-colors">
                    {noteAiLoading ? <Loader2 size={11} className="animate-spin" /> : <Sparkles size={11} />} Generate Note with AI
                  </button>
                </div>
                <textarea value={newNoteContent} onChange={(e) => setNewNoteContent(e.target.value)} placeholder="Write your notes..." rows={8} className={inputCls + " resize-none"} />
              </div>
              <div><label className={labelCls}>Tags (comma-separated)</label><input value={newNoteTags} onChange={(e) => setNewNoteTags(e.target.value)} placeholder="e.g. midterm, chapter-3, formulas" className={inputCls} /></div>
              <div className="flex gap-2">
                <button onClick={handleCreateNote} className="px-4 py-2 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors">Save Note</button>
                <button onClick={() => setShowNewNote(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
              </div>
            </motion.div>
          )}
          {notesLoading ? (
            <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 animate-pulse"><div className="h-3 bg-slate-200 dark:bg-slate-700 rounded w-1/3 mb-2" /><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded w-2/3" /></div>)}</div>
          ) : notes.length === 0 ? (
            <EmptyState icon={StickyNote} title="No notes yet" description="Create notes to help organize study material" action="New Note" onAction={() => setShowNewNote(true)} />
          ) : (
            notes.map((note) => (
              <motion.div key={note._id || note.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {note.isAiGenerated && <span className="text-[9px] font-bold text-purple-500 bg-purple-50 dark:bg-purple-500/10 px-1.5 py-0.5 rounded-full flex-shrink-0">AI</span>}
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => handleSummarizeNote(note)} disabled={noteSummaryLoading === (note._id || note.id)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-green-500 transition-colors disabled:opacity-50" title="Summarize with AI">
                      {noteSummaryLoading === (note._id || note.id) ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {(note.tags || []).map((tag, i) => (
                      <span key={i} className="text-[9px] font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-full">#{tag}</span>
                    ))}
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400">
                    {note.authorId?.name && <span>by {note.authorId.name}</span>}
      ...
      {activeTab === "materials" && (
        <div className="space-y-3">
          {isInstructor && (
            <button onClick={() => setShowNewMaterial(!showNewMaterial)} className="flex items-center gap-2 w-full p-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-500 hover:border-green-400 hover:text-green-600 transition-colors">
          ...
          {showNewMaterial && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={sectionCls}>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelCls}>Title *</label><input value={newMat.title} onChange={(e) => setNewMat({ ...newMat, title: e.target.value })} placeholder="Material title" className={inputCls} /></div>
                <div><label className={labelCls}>Type</label>
                  <select value={newMat.type} onChange={(e) => setNewMat({ ...newMat, type: e.target.value })} className={inputCls + " appearance-none"}>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div><label className={labelCls}>Week #</label><input type="number" min={0} value={newMat.weekNumber} onChange={(e) => setNewMat({ ...newMat, weekNumber: parseInt(e.target.value) || 0 })} className={inputCls} /></div>
                <div><label className={labelCls}>Category</label><input value={newMat.category} onChange={(e) => setNewMat({ ...newMat, category: e.target.value })} placeholder="e.g. Reading" className={inputCls} /></div>
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={newMat.isRequired} onChange={(e) => setNewMat({ ...newMat, isRequired: e.target.checked })} className="w-4 h-4 rounded border-slate-300 text-green-500 focus:ring-green-500" />
                    <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Required</span>
                  </label>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={handleCreateMaterial} className="px-4 py-2 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors">Add Material</button>
                <button onClick={() => setShowNewMaterial(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
              </div>
            </motion.div>
          )}
          {materialsLoading ? (
            ...
            <EmptyState icon={Layers} title="No materials yet" description="Upload course materials, links, and resources" action="Add Material" onAction={() => setShowNewMaterial(true)} />
          ) : (
            Object.entries(
              materials.reduce((acc, mat) => { const wk = mat.weekNumber || 0; if (!acc[wk]) acc[wk] = []; acc[wk].push(mat); return acc; }, {})
            ).sort(([a], [b]) => Number(a) - Number(b)).map(([week, mats]) => (
                ...
                {mats.map((mat) => {
                  ...
                  return (
                    <motion.div key={mat._id || mat.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        <div className="w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-shrink-0"><MIcon className="w-4 h-4 text-slate-500" /></div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <h4 className="text-sm font-bold text-slate-900 dark:text-white truncate">{mat.title}</h4>
                            {mat.isRequired && <span className="text-[9px] font-bold text-red-500 bg-red-50 dark:bg-red-500/10 px-1.5 py-0.5 rounded-full">Required</span>}
                            {mat.category && <span className="text-[9px] font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-full">{mat.category}</span>}
                          </div>
                          {mat.description && <p className="text-[11px] text-slate-500 line-clamp-2 mb-1">{mat.description}</p>}
                          {mat.aiSummary && <p className="text-[10px] text-green-600 dark:text-green-400 italic mt-1">AI Summary: {mat.aiSummary}</p>}
                          <div className="flex items-center gap-2 mt-1">
                            {mat.url && <a href={mat.url} target="_blank" rel="noopener noreferrer" className="text-[10px] font-semibold text-green-600 hover:text-green-700 flex items-center gap-0.5" onClick={(e) => e.stopPropagation()}>Open <ExternalLink className="w-3 h-3" /></a>}
                          </div>
                        </div>
                      </div>
                    </motion.div>
              ...
              <div key={student.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-slate-900 dark:text-white">{student.completed}/{student.totalAssignments}</p>
                    <p className="text-[10px] text-slate-500">completed</p>
                  </div>
                </div>
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-green-400 to-emerald-500 transition-all duration-500" style={{ width: `${student.avgProgress}%` }} />
                </div>
                <div className="flex items-center justify-between mt-1.5">
                  <span className="text-[10px] text-slate-500">{student.avgProgress}% avg progress</span>
                  <span className="text-[10px] text-slate-500">{student.totalTimeMinutes}min spent</span>
                </div>
              </div>
            ))
          )}
        ...
        <div className="space-y-4">
          ...
          <div className={sectionCls}>
              ...
              <div className="flex gap-1.5">
                {daysOfWeek.map((day) => (
                  <button key={day} onClick={() => {
                    ...
                    setSettingsForm({ ...settingsForm, schedule: { ...settingsForm.schedule, days: newDays } });
                  }}
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${(settingsForm.schedule.days || []).includes(day) ? "bg-green-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700"}`}>
                    {day}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className={labelCls}>End Time</label><input type="time" value={settingsForm.schedule.endTime || ""} onChange={(e) => setSettingsForm({ ...settingsForm, schedule: { ...settingsForm, schedule: { ...settingsForm.schedule, endTime: e.target.value } } })} className={inputCls} /></div>
          ...
          <div className={sectionCls}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={labelCls}>Academic Level</label>
                <select value={settingsForm.academicLevel} onChange={(e) => setSettingsForm({ ...settingsForm, academicLevel: e.target.value })} className={inputCls + " appearance-none"}>
                  <option value="highschool">High School</option><option value="undergraduate">Undergraduate</option><option value="graduate">Graduate</option><option value="phd">PhD</option><option value="professional">Professional</option>
                </select>
              </div>
            </div>
            <div><label className={labelCls}>Grading Scheme</label>
              <select value={settingsForm.gradingScheme} onChange={(e) => setSettingsForm({ ...settingsForm, gradingScheme: e.target.value })} className={inputCls + " appearance-none"}>
                <option value="percentage">Percentage</option><option value="letter">Letter Grades</option><option value="passfail">Pass/Fail</option><option value="gpa">GPA Scale</option>
              </select>
            </div>
          </div>
          ...
          <div className={sectionCls}>
            ...
            <div className="flex gap-2">
              <input value={settingsForm.newPrereq || ""} onChange={(e) => setSettingsForm({ ...settingsForm, newPrereq: e.target.value })} placeholder="Add prerequisite..." className={inputCls + " flex-1"} onKeyDown={(e) => { if (e.key === "Enter" && settingsForm.newPrereq?.trim()) { setSettingsForm({ ...settingsForm, prerequisites: [...(settingsForm.prerequisites || []), settingsForm.newPrereq.trim()], newPrereq: "" }); } }} />
              <button onClick={() => { if (settingsForm.newPrereq?.trim()) setSettingsForm({ ...settingsForm, prerequisites: [...(settingsForm.prerequisites || []), settingsForm.newPrereq.trim()], newPrereq: "" }); }} className="px-3 py-2 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors"><Plus className="w-4 h-4" /></button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {(settingsForm.prerequisites || []).map((prereq, i) => (
                <span key={i} className="flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[11px] font-medium text-slate-700 dark:text-slate-300">
                  {prereq}
                  <button onClick={() => setSettingsForm({ ...settingsForm, prerequisites: settingsForm.prerequisites.filter((_, idx) => idx !== i) })} className="text-slate-400 hover:text-red-500"><X className="w-3 h-3" /></button>
                </span>
              ))}
              {(!settingsForm.prerequisites || settingsForm.prerequisites.length === 0) && <p className="text-[11px] text-slate-400 italic">No prerequisites</p>}
            </div>
          </div>
          ...
          <div className={sectionCls}>
            ...
            {[
              ...
              <div key={key} className="flex items-center justify-between">
                ...
                <button onClick={() => setSettingsForm({ ...settingsForm, settings: { ...settingsForm.settings, [key]: !settingsForm.settings[key] } })}
                  className={toggleCls(settingsForm.settings[key])}><span className={toggleDot(settingsForm.settings[key])} /></button>
              </div>
            ))}
          </div>
          ...
          <div className={sectionCls}>
            ...
            {[
              { key: "allowStudentPosts", label: "Allow Student Posts" },
              { key: "requireApproval", label: "Require Approval" },
              { key: "showGradesToStudents", label: "Show Grades to Students" },
            ...
            <div>
              ...
              <input type="number" min={0} max={100} value={settingsForm.settings.latePenaltyPercent || 0} onChange={(e) => setSettingsForm({ ...settingsForm, settings: { ...settingsForm.settings, latePenaltyPercent: parseInt(e.target.value) || 0 } })} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Max File Size (MB)</label>
              <input type="number" min={1} max={100} value={settingsForm.settings.maxFileSizeMB || 50} onChange={(e) => setSettingsForm({ ...settingsForm, settings: { ...settingsForm.settings, maxFileSizeMB: parseInt(e.target.value) || 50 } })} className={inputCls} />
            </div>
          </div>
          ...
          <div className={sectionCls}>
            ...
            <div className="flex items-center gap-2">
              <code className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg font-mono text-sm font-bold tracking-wider">{classroom.inviteCode}</code>
              <button onClick={() => { navigator.clipboard.writeText(classroom.inviteCode); toast.success("Copied!"); }} className="p-2 text-slate-400 hover:text-green-500 transition-colors"><Copy className="w-4 h-4" /></button>
            </div>
          </div>

          <button onClick={handleSaveSettings} disabled={settingsSaving} className="w-full py-2.5 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 disabled:opacity-50 transition-colors">
          ...
          <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl p-5 space-y-3">
            ...
            <p className="text-xs text-red-500/70">Archiving a classroom will hide it from all users and remove student enrollments. This action can be reversed by an admin.</p>
            <button onClick={handleDeleteClassroom} className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-lg text-xs font-semibold hover:bg-red-600 transition-colors">
              <Archive className="w-3.5 h-3.5" /> Archive Classroom
            </button>
          </div>
        </div>
      )}
...
export default function ClassroomDashboard() {
  ...
  if (showCreate) {
    return <CreateClassroomForm onClose={() => setShowCreate(false)} onCreated={(c) => { setClassrooms([c, ...classrooms]); setShowCreate(false); }} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {!isInstructor && (
            <button onClick={() => setShowJoin(!showJoin)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${showJoin ? "bg-green-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"}`}>
              <UserPlus className="w-3.5 h-3.5" /> Join
            </button>
          )}
          {isInstructor && (
            <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 text-white rounded-lg text-xs font-semibold hover:bg-green-600 transition-colors">
        ...
        <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl p-3">
          ...
          <div className="space-y-1.5">
            {upcomingDeadlines.map((d) => {
              const hoursLeft = (new Date(d.dueDate) - new Date()) / (1000 * 60 * 60);
              ...
              return (
                <button key={d.id} onClick={() => { const cls = classrooms.find((c) => c.id === d.classroomId); if (cls) setSelectedClassroom(cls); }} className="flex items-center gap-2 w-full text-left group">
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${urgent ? "bg-red-400 animate-pulse" : "bg-amber-400"}`} />
                  <span className="text-[11px] text-amber-800 dark:text-amber-300 font-medium truncate flex-1 group-hover:underline">{d.title}</span>
                  <span className="text-[10px] text-amber-600 dark:text-amber-500 flex-shrink-0">{urgent ? `${Math.round(hoursLeft)}h` : `${Math.round(hoursLeft / 24)}d`}</span>
                </button>
              );
            })}
          </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {[1, 2, 3].map((i) => <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 animate-pulse"><div className="flex items-start justify-between mb-3"><div className="w-9 h-9 rounded-lg bg-slate-200 dark:bg-slate-700" /><div className="w-5 h-5 rounded bg-slate-200 dark:bg-slate-700" /></div><div className="h-3 bg-slate-200 dark:bg-slate-700 rounded w-2/3 mb-2" /><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded w-1/3 mb-3" /><div className="flex gap-3 pt-2 border-t border-slate-100 dark:border-slate-800"><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded w-16" /><div className="h-2 bg-slate-200 dark:bg-slate-700 rounded w-16" /></div></div>)}
        </div>
      ) : classrooms.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          {isInstructor ? (
            <button onClick={() => setShowCreate(true)} className="px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 transition-colors">Create Classroom</button>
          ) : (
