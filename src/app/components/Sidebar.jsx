...
export default function Sidebar() {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 1024) {
        // lg breakpoint
        setIsCollapsed(false); // Fully expanded
      } else if (window.innerWidth >= 768) {
        // md breakpoint
        setIsCollapsed(true); // Minimized (icons only)
      } else {
        setIsCollapsed(true); // Hidden on small screens
      }
    };
  ...
  return (
    <>
      ...
      {/* Toggle Button - Always visible, lower on mobile */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="fixed left-4 top-6 sm:top-[80%] md:top-6 w-8 h-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors z-20"
      >
        {isCollapsed ? (
      ...
      {/* Sidebar Content */}
      <motion.div
        className="fixed md:sticky top-0 h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden z-1 lg:static"
