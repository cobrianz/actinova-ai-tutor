...
export default function PremiumCourses() {
  ...
  const buildShareLink = (course) => {
    ...
    return `${origin}/dashboard/premium-courses?courseSlug=${encodeURIComponent(course.slug)}`;
  };
  ...
  async function handleStartLearning(course) {
    try {
      ...
      if (course.isTrending && !course.id?.startsWith("trending-")) {
        ...
        router.push(`/dashboard/learn/${encodeURIComponent(course.title)}?${nextParams.toString()}`);
        return;
      }
      ...
      router.push(`/dashboard/learn/${encodeURIComponent(data.topic)}?${nextParams.toString()}`);
    } catch (error) {
