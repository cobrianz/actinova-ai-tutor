    ...
    try {
      ...
      if (responseData.courseId || responseData.success) {
        const safeTopic = topic.title
          ...
          .replace(/\s+/g, "-");
        router.push(
          `/dashboard/learn/${encodeURIComponent(safeTopic)}?format=course&difficulty=${topic.difficulty || "beginner"}&originalTopic=${encodeURIComponent(topic.title)}`
        );
      }
    } catch (error) {
  ...
  return (
    <div className="px-0 py-6">
      ...
      <UpgradeModal
        ...
        featureName={selectedCategoryForModal?.name}
        description={`Unlock personalized courses and expert-led curricula in the ${selectedCategoryForModal?.name || "Premium"} category.`}
      />
