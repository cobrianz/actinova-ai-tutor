  ...
  const continueGeneration = async ({ premium = false } = {}) => {
    ...
    try {
      ...
      if (cached && !premium) {
        ...
        router.push(
          `/dashboard/learn/${encodeURIComponent(subject)}?format=${format}&difficulty=${difficulty}`
        );
        return;
      }
    } catch { }
    ...
    setShowLoader(true);
    router.push(
      `/dashboard/learn/${encodeURIComponent(subject)}?format=${format}&difficulty=${difficulty}${premium ? "&premiumRequested=true" : ""}`
    );
