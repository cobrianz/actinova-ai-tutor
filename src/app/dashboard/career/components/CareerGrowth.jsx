...
const CareerGrowth = () => {
    ...
    const setSubTab = (tool, extraParams = {}) => {
        ...
        const routeMap = {
            resume: "/dashboard/career/resume",
            interview: "/dashboard/career/interview",
            skillgap: "/dashboard/career/skills",
            network: "/dashboard/career/network",
        };
        if (routeMap[tool]) {
            ...
            router.push(`${routeMap[tool]}${qs ? `?${qs}` : ""}`);
        } else {
    ...
    const handleGenerateCourse = async (topicTitle) => {
        ...
        try {
            ...
            if (responseData.courseId || responseData.success) {
                ...
                router.push(`/dashboard/learn/${encodeURIComponent(safeTopic)}?format=course&difficulty=beginner`);
            }
        } catch (error) {
