...
import { TYPE_CONFIG, getDueStatus } from "./constants";
...
export default function AssignmentsTab({ classroomState }) {
  const {
    ...
    handleUnforkContent, handleForkContent,
  ...
  return (
    <div className="space-y-3">
      {submissionsAssignment ? (<>
        ...
        {isInstructor && (
          <div className="space-y-3">
            <button onClick={() => { setEditingAssignment(null); setShowCreateAssignment(!showCreateAssignment); }} className={`flex items-center gap-2 w-full p-3 border-2 rounded-xl text-sm transition-colors ${showCreateAssignment ? "border-green-400 bg-green-50 dark:bg-green-500/10 text-green-600" : "border-dashed border-slate-200 dark:border-slate-700 text-slate-500 hover:border-green-400 hover:text-green-600 bg-white dark:bg-slate-900"}`}><Plus className="w-4 h-4" /> {showCreateAssignment ? "Close" : "Add Assignment"}</button>
            {showCreateAssignment && <CreateAssignmentPanel classroomId={classroom.id} classroomName={classroom.name} onClose={() => { setShowCreateAssignment(false); setEditingAssignment(null); }} onCreated={handleAssignmentSaved} editAssignment={editingAssignment} forkedContent={forkedContent || []} materials={materials || []} discussions={discussions || []} courseModules={courseModules || []} durationWeeks={classroom.durationWeeks || 12} />}
          </div>
        )}
