...
async function handleGet(request, { params }) {
  ...
  if (type === "all" || type === "course") {
    ...
    const courseQuery = { $or: [{ createdBy: userId }, { createdBy: String(user._id) }] };
    if (q) courseQuery.title = { $regex: q, $options: "i" };
    const courses = await db.collection("courses").find(courseQuery).project({ title: 1, level: 1, totalModules: 1, totalLessons: 1, description: 1, createdAt: 1 }).sort({ createdAt: -1 }).limit(100).toArray();
