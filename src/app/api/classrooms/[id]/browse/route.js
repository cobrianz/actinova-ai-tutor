...
async function handleGet(request, { params }) {
  ...
  if (type === "all" || type === "course") {
    const searchFilter = q
      ...
      : {};
    const courses = await db.collection("library").find({
      $and: [
        { $or: [{ userId: userId }, { userId: userIdStr }] },
        { $or: [{ format: "course" }, { type: "course" }] },
        searchFilter,
      ],
    }).project({ title: 1, level: 1, totalModules: 1, totalLessons: 1, description: 1, topic: 1, difficulty: 1, userId: 1, createdAt: 1 }).sort({ createdAt: -1 }).limit(100).toArray();
  ...
  if (type === "all" || type === "quiz") {
    ...
    const userQuery = {
      $and: [
        { $or: [{ createdBy: userId }, { createdBy: userIdStr }, { userId: userId }, { userId: userIdStr }] },
        searchFilter,
      ],
    };

    const Quiz = (await import("@/models/Quiz")).default;
    const quizzesFromTests = await Quiz.find(userQuery).select("title course questions createdAt createdBy").sort({ createdAt: -1 }).limit(100).lean();

    const examsFromExams = await db.collection("exams").find(userQuery).project({ title: 1, topic: 1, questions: 1, createdAt: 1, userId: 1 }).sort({ createdAt: -1 }).limit(100).toArray();
  ...
  if (type === "all" || type === "flashcard") {
    const searchFilter = q
      ? { $or: [{ title: { $regex: q, $options: "i" } }, { topic: { $regex: q, $options: "i" } }] }
      : {};
    const flashcards = await db.collection("cardSets").find({
      $and: [
        { $or: [{ userId: userId }, { userId: userIdStr }] },
        searchFilter,
      ],
    }).project({ title: 1, topic: 1, difficulty: 1, totalCards: 1, userId: 1, createdAt: 1 }).sort({ createdAt: -1 }).limit(100).toArray();
  ...
  if (type === "all" || type === "report") {
    ...
    }).project({ title: 1, topic: 1, format: 1, summary: 1, userId: 1, createdAt: 1 }).sort({ createdAt: -1 }).limit(100).toArray();
